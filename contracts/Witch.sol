...
/_/  \____/_/ /_/ /_/_.___/  /_/   /_/_/ /_/\__,_/_/ /_/\___/\___/
    ...
    http://witch.finance
*/
contract Witch is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    /* ================= Taxation =============== */
    // Address of the Oracle
    address public witchOracle;
    ...
    /**
     * @notice Constructs the WITCH ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("WITCH", "WITCH") {
        // Mints 1 WITCH to contract creator for initial pool setup
    ...
    function _getWitchPrice() internal view returns (uint256 _witchPrice) {
        try IOracle(witchOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Witch: failed to fetch WITCH price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _witchPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_witchPrice >= taxTiersTwaps[tierId]) {
    ...
    function setWitchOracle(address _witchOracle) public onlyOperatorOrTaxOffice {
        require(_witchOracle != address(0), "oracle address cannot be 0 address");
        witchOracle = _witchOracle;
    }
    ...
    /**
     * @notice Operator mints WITCH to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of WITCH to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentWitchPrice = _getWitchPrice();
            currentTaxRate = _updateTaxRate(currentWitchPrice);
            if (currentWitchPrice < burnThreshold) {
                burnTax = true;
            }
        }
